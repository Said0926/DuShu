...
# Здесь лежит кэш озвучки. Аудио — производный артефакт: его можно потерять и
# пересоздать, поэтому оно и хранится файлами, а не в базе. В базе остаётся то,
# что без аудио бесполезно, — тайминги и метаданные.
# Внимание при деплое: каталог должен стать именованным томом или уехать в S3,
# иначе пересоздание контейнера сотрёт весь кэш.
MEDIA_URL = "media/"
...
# Провайдер озвучки — тем же способом, что и провайдер перевода: путём к классу.
# EdgeTTSProvider бесплатен и не требует ключа, DummyTTSProvider отдаёт тишину
# и позволяет работать над страницей без сети.
TTS_PROVIDER = env(
    "TTS_PROVIDER",
    default="apps.tts.providers.edge.EdgeTTSProvider",
)

# Голос один и в интерфейсе не выбирается: в DESIGN.md такого контрола нет,
# а один голос упрощает ключ кэша. Список доступных: edge-tts --list-voices.
TTS_VOICE = env("TTS_VOICE", default="zh-CN-XiaoxiaoNeural")

# Сколько предложений озвучивать одновременно. Текст озвучивается целиком при
# открытии страницы, и последовательный синтез сотни предложений занял бы минуты.
# Четыре — компромисс: ускоряет в разы, но не выглядит для чужого сервиса
# как попытка его положить.
TTS_CONCURRENCY = 4
...
# Озвучка денег не стоит: edge-tts бесплатен. Лимит здесь стережёт не бюджет,
# а нашу репутацию у неофициального эндпоинта и процессор — поэтому числа те же,
# что у обработки текста, а не мягче.
RATELIMIT_AUDIO_GUEST = "5/h"
RATELIMIT_AUDIO_USER = "30/h"
