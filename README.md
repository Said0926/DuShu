...
- [Словари](#словари)
- [Каталог текстов HSK](#каталог-текстов-hsk)
...
уровней HSK с пятьюдесятью готовыми текстами, вход по email и через Google, ограничение частоты
запросов, настройки отображения, которые следуют за пользователем между устройствами.

Стек: Python 3.12, Django 5.2, PostgreSQL 16, Django templates и vanilla JS без фронтенд-фреймворка,
Docker, pytest, ruff, GitHub Actions. 507 тестов.
...
Готово — http://localhost:8000. Последняя команда раскладывает по уровням HSK пятьдесят
готовых текстов, так что библиотека не встретит пустыми полками.
...
```bash
# Админка: оттуда правят общие тексты поштучно, если не хочется трогать файл каталога.
...
В репозитории лежат пятьдесят готовых текстов — по десять на каждый уровень, от четырёх
предложений на HSK 1 до двенадцати на HSK 5. Они общие: принадлежат не пользователю, а всем
сразу, и видны каждому, кто зашёл в библиотеку.

```bash
docker compose exec web python manage.py load_hsk_texts
```

Команда безопасна при повторном запуске: текст узнаётся по отпечатку содержимого. Правка
заголовка подхватится следующим запуском, а правка самого текста создаст новый — отпечаток
другой. Убрать то, чего в файле больше нет, можно флагом `--prune`. Он отдельный, потому что
удаление общего текста уносит с собой отметки о прочтении всех читателей. Флаг `--level`
грузит один уровень и не трогает соседние.

Сами тексты — `apps/library/data/hsk_texts.json`, обычный список:
...
и правка опечатки стоила бы новой миграции каждый раз.

Словарь каждого текста не выходит за пределы своего уровня. Для HSK 1 и HSK 2 это проверено
машинно — списки в 150 и 300 слов выписываются целиком; для старших уровней слова
просматривались глазами, потому что выдуманный список на 2500 слов хуже отсутствующего.
Проверять приходится не токены сегментатора, а их разложение: jieba склеивает соседние слова
в одно (今天上午, 八块钱), и такого «слова» нет ни в одном списке, хотя составлено оно целиком
из слов уровня.
...
Переводит и озвучивает весь каталог заранее. После этого общие тексты открываются мгновенно
и не тратят почасовой лимит ни у кого: лимит считает работу, отправленную провайдеру, а её
после прогрева нет. Цена — около 12 000 символов из месячной квоты DeepL в 500 000 и пара
минут синтеза; на выходе 23 минуты озвучки и примерно 9 МБ в `media/`.

Отказ одного текста не останавливает прогон — неофициальный эндпоинт озвучки временами
отвечает 503, и пропущенное догревается повторным запуском. И помнить про `media/`: это не
именованный том, пересоздание контейнера сотрёт аудио.
